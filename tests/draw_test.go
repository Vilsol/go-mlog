...
func TestDraw(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "DrawClear",
			input:  TestMain(`m.DrawClear(1, 2, 3)`),
			output: `draw clear 1 2 3`,
		},
		{
			name:   "DrawColor",
			input:  TestMain(`m.DrawColor(1, 2, 3, 4)`),
			output: `draw color 1 2 3 4`,
		},
		{
			name:   "DrawStroke",
			input:  TestMain(`m.DrawStroke(1)`),
			output: `draw stroke 1`,
		},
		{
			name:   "DrawLine",
			input:  TestMain(`m.DrawLine(1, 2, 3, 4)`),
			output: `draw line 1 2 3 4`,
		},
		{
			name:   "DrawRect",
			input:  TestMain(`m.DrawRect(1, 2, 3, 4)`),
			output: `draw rect 1 2 3 4`,
		},
		{
			name:   "DrawLineRect",
			input:  TestMain(`m.DrawLineRect(1, 2, 3, 4)`),
			output: `draw lineRect 1 2 3 4`,
		},
		{
			name:   "DrawPoly",
			input:  TestMain(`m.DrawPoly(1, 2, 3, 4, 5)`),
			output: `draw poly 1 2 3 4 5`,
		},
		{
			name:   "DrawLinePoly",
			input:  TestMain(`m.DrawLinePoly(1, 2, 3, 4, 5)`),
			output: `draw linePoly 1 2 3 4 5`,
		},
		{
			name:   "DrawTriangle",
			input:  TestMain(`m.DrawTriangle(1, 2, 3, 4, 5, 6)`),
		...
		{
			name:   "DrawFlush",
			input:  TestMain(`m.DrawFlush("display1")`),
